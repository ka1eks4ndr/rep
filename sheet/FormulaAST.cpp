...
public:
    ...
    virtual double Evaluate(std::function<const CellInterface*(Position pos)>& get_cell ) const = 0;
...
public:
    ...
    double Evaluate(std::function<const CellInterface*(Position pos)>& get_cell) const override {
        auto left = lhs_->Evaluate(get_cell);
        auto right = rhs_->Evaluate(get_cell);
        if (type_ == Type::Add) {
            if (MAX - right < left || 
                MAX - left < right) {
            ...
            if (right == 0 ) {
                throw FormulaError (FormulaError::Category::Div0); 
            } else if (auto result = left / right; left != result * right) {
                throw FormulaError (FormulaError::Category::Div0); 
            } else {
                return left / right;
            }
            ...
            throw std::logic_error ("Bad BinaryOpExpr");
        }
    }
...
public:
    ...
    double Evaluate(std::function<const CellInterface*(Position pos)>& get_cell) const override {
        if (type_ == Type::UnaryMinus) {
            return -operand_->Evaluate(get_cell);

        } else if (type_ == Type::UnaryPlus) {
            return operand_->Evaluate(get_cell);
        } else {
            throw std::logic_error ("Bad Unary operation");
        }
    }
...
public:
    ...
    double Evaluate(std::function<const CellInterface*(Position pos)>& get_cell) const override {
        ...
        throw std::logic_error("cell evaluate error");
    }
...
double FormulaAST::Execute(std::function<const CellInterface*(Position pos)>& get_cell) const {
