...
private: 
    ...
    std::unique_ptr<Statement> argument_ = nullptr;
...
protected:
    const std::unique_ptr<Statement>& GetLhs () {
        return lhs_;
    }

    const std::unique_ptr<Statement>& GetRhs () {
        return rhs_;
    }
private:
...
public:
    ...
    runtime::ObjectHolder Execute(runtime::Closure& closure, runtime::Context& context) override;
private:
    std::unique_ptr<Statement> condition_ = nullptr;
    std::unique_ptr<Statement> if_body_ = nullptr;
    std::unique_ptr<Statement> else_body_ = nullptr;
};
