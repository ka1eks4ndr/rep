...
namespace parse {
    ...
    public:
        ...
        const T& Expect() const {
            ...
            if (!(vtoken_.back().Is<T>())) {
                throw LexerError("wrong Type");
            }
            return vtoken_.back().As<T>();
        }
    ...
    private:
        ...
        Token ParserString(char quotes);
        Token ParserGeneral();
        Token ParserDigit();
        Token ParserLiteral();
        Token ParserComparison();
        Token GetKeyWords(int KeyWord);

        bool TokenChar(char c);
        std::pair<bool, int> IsKeyWord(std::string_view word);

        std::istream& input_;
        std::vector<Token>vtoken_;
        bool new_line_ = true;
        int indent_ = 0;
        int dedent_debt = 0;
    };
